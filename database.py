...
ball_data = ball_data[["inning","over","ball","batsman","non_striker","bowler", "extra_runs", "boundary"]]
...
cursor.execute('CREATE TABLE IF NOT EXISTS balls (inning int, over int, ball int, batsman nvarchar(50), non_striker nvarchar(50), bowler nvarchar(50), extra_runs int, boundary boolean)')
...
for ball in ball_data.itertuples():
    cursor.execute('INSERT INTO balls (inning, over, ball, batsman, non_striker, bowler, extra_runs, boundary) VALUES (?,?,?,?,?,?,?,?)',
                (ball.inning, 
                ball.over,
                ball.ball,
                ball.batsman,
